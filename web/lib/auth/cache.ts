 ...
 * Get item from cache
 */
export function getCached<T>(key: string, namespace: string = 'default'): T | null {
 ...
 */
export function deleteCached(key: string, namespace: string = 'default'): boolean {
  const cacheKey = `${namespace}:${key}`;
...
function evictOldestEntries(count: number): number {
  const entries = Array.from(cacheStore.entries()).sort((a, b) => a[1].timestamp - b[1].timestamp);

  let evicted = 0;
  for (let i = 0; i < Math.min(count, entries.length); i++) {
    const entry = entries[i];
    if (entry) {
      cacheStore.delete(entry[0]);
      evicted++;
    }
  }
