...
class PerformanceMonitor {
  ...
  private checkBudget(entry: PerformanceEntry): void {
    const budget = this.budgets.find(
      (b) => b.type === entry.type && (b.name === entry.name || b.name === `${entry.type}-request`)
    );
  ...
  private getMedian(values: number[]): number {
    ...
    return sorted.length % 2 === 0 ? ((left ?? 0) + (right ?? 0)) / 2 : (sorted[mid] ?? 0);
  }
...
export function usePerformanceMonitor(componentName: string) {
  ...
  return () => {
    const duration = performance.now() - startTime;
    performanceMonitor.measure(componentName, () => {}, 'render', { duration });
  };
}
