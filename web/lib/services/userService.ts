...
export class UserService {
   ...
   */
  async createUser(input: CreateUserInput, currentUserRole: string, createdBy?: string) {
    // 1. Role Escalation / Protection Logic
    if ((input.role as string) === 'super_admin' && currentUserRole !== 'super_admin') {
      throw new AuthorizationError('Bạn không có quyền tạo tài khoản super_admin');
    }
    ...
    if (
      ...
      throw new AuthorizationError('Bạn không có quyền tạo tài khoản với vai trò này');
    }
    ...
    const fullName =
      input.full_name ||
      (firstName && lastName ? `${lastName} ${firstName}`.trim() : firstName || lastName);
    ...
    if (!email) {
      if (input.role === 'student') {
        ...
        const domain = input.role === 'parent' ? '@parent.bhedu.vn' : '@bhedu.vn';
    ...
    try {
      ...
      const isManagedEmail = MANAGED_DOMAINS.some((domain) => email.toLowerCase().endsWith(domain));

      const recipientEmail =
        isManagedEmail && input.personal_email
      ...
      if (recipientEmail) {
        ...
        await sendEmail({
          to: recipientEmail,
          subject: 'Chào mừng đến với BH-EDU - Thông tin tài khoản của bạn',
          ...emailContent,
        });
      }
  ...
  async deleteUser(id: string) {
    ...
    if (user.user_id) {
      const { error: authError } = await this.supabase.auth.admin.deleteUser(user.user_id);
      if (authError) {
    ...
    await this.supabase.from('teacher_profiles').delete().eq('profile_id', id);
    await this.supabase.from('student_profiles').delete().eq('profile_id', id);
  ...
  static async createUser(input: CreateUserInput, currentUserRole: string, createdBy?: string) {
