...
describe('EnrollmentRepository', () => {
  ...
  describe('findByStudent', () => {
    it('should fetch enrollments with detailed class and teacher info', async () => {
      ...
      expect(selectMock).toHaveBeenCalledWith(expect.stringContaining('teacher:profiles!classes_teacher_id_fkey'))
    ...
    it('should throw error on database failure', async () => {
      ...
      await expect(repository.findByStudent('id')).rejects.toThrow('Failed to fetch student enrollments: DB Error')
    })
  })
  ...
  describe('transferStudent', () => {
    it('should delete existing enrollment and create new one', async () => {
      ...
      const newEnrollment = { id: 'enr-new', student_id: studentId, class_id: toClassId, status: 'enrolled' }
    ...
    it('should throw error if student is not enrolled in source class', async () => {
      ...
      await expect(repository.transferStudent('s1', 'c1', 'c2'))
        .rejects.toThrow('Student is not enrolled in the source class')
    })
  })
