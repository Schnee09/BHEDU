...
import { Menu, Search, Check, Palette, Droplets, Wind, Sun, Moon, Plus } from 'lucide-react';
import { useCustomization, AccentColor, UIDensity, ThemeMode } from '@/contexts/CustomizationContext'
...
function SettingsPageContent() {
  ...
  const handleSaveYear = async (yearData: Partial<AcademicYear>) => {
    ...
    toast.success('Thành công', `Đã ${selectedYear ? 'cập nhật' : 'thêm'} năm học ${yearData.name}`);
    fetchAcademicYears();
  }
  ...
  const tabs = [
    ...
    { id: 'notifications', label: 'Thông báo', icon: Icons.Notifications, color: 'text-indigo-500' },
    { id: 'finance', label: 'Tài chính', icon: Icons.Finance, color: 'text-orange-500' },
  ]

  const isSearching = searchQuery.length > 0;

  const filteredSettings = settings.filter(s => 
    isSearching ? true : s.category === activeTab
  ).filter(s => 
  ...
  return (
    <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-8 animate-fade-in relative">
      {/* Background decoration with dynamic accent color */}
      <div className="fixed top-0 right-0 w-[600px] h-[600px] rounded-full blur-[120px] opacity-[0.03] dark:opacity-[0.05] pointer-events-none -z-10" style={{ backgroundColor: 'var(--color-primary)' }} />
      
      {/* Premium Header */}
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-1">
          <div className="inline-flex px-3 py-1 bg-amber-500/10 border border-amber-500/20 rounded-full mb-2" style={{ borderColor: 'var(--color-primary-20)', backgroundColor: 'var(--color-primary-10)' }}>
             <span className="text-[10px] font-black uppercase tracking-widest text-stone-600 dark:text-stone-300" style={{ color: 'var(--color-primary)' }}>Hệ thống BH-EDU</span>
          </div>
            ...
            <nav className="flex flex-row lg:flex-col overflow-x-auto lg:overflow-visible gap-1 scroll-hide p-1">
              {tabs.map((tab) => (
                <button
                  ...
                  style={activeTab === tab.id ? { backgroundColor: 'var(--color-primary-10)', color: 'var(--color-primary)' } : {}}
                >
                  ...
                  {activeTab === tab.id && (
                    <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-6 bg-amber-500 rounded-full" style={{ backgroundColor: 'var(--color-primary)' }} />
                  )}
                </button>
              ))}
            </nav>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-fade-in-up">
               ...
               <Card className="rounded-[40px] overflow-hidden border-stone-100 dark:border-white/5 glass-premium shadow-xl shadow-black/5">
                 <CardHeader className="bg-gradient-to-br from-amber-500/5 to-transparent border-b border-stone-100 dark:border-white/5 py-6 px-8">
                    <div className="flex items-center gap-3">
                       <span className="p-2.5 bg-amber-500/10 rounded-2xl"><Icons.Layout className="w-5 h-5 text-amber-500" /></span>
                       <h2 className="text-xl font-black tracking-tight">Màu chủ đạo</h2>
                    </div>
                 </CardHeader>
                 <CardBody className="p-8 space-y-8">
                    <p className="text-sm text-stone-500 font-medium">Chọn tông màu đặc trưng cho giao diện dashboard của bạn.</p>
                    <div className="grid grid-cols-4 gap-4">
                       {[
                         ...
                         >
                            <div className={cn("w-10 h-10 rounded-2xl shadow-lg ring-4 ring-white/20", c.color)} />
               ...
               <Card className="rounded-[40px] overflow-hidden border-stone-100 dark:border-white/5 glass-premium shadow-xl shadow-black/5">
                 <CardHeader className="bg-gradient-to-br from-blue-500/5 to-transparent border-b border-stone-100 dark:border-white/5 py-6 px-8">
                    <div className="flex items-center gap-3">
                       <span className="p-2.5 bg-blue-500/10 rounded-2xl"><Icons.Columns className="w-5 h-5 text-blue-500" /></span>
                    ...
                    <div className="space-y-6 pt-2">
                       <div className="space-y-3">
                          <div className="flex justify-between items-end px-1">
                             <span className="text-[10px] font-black text-stone-400 uppercase tracking-widest">Kính mờ (Blur)</span>
                             <span className="text-sm font-black text-stone-900 dark:text-white">{blurStrength}<span className="text-[10px] opacity-40 ml-0.5">PX</span></span>
                          </div>
                       ...
                       <div className="space-y-3">
                          <div className="flex justify-between items-end px-1">
                             <span className="text-[10px] font-black text-stone-400 uppercase tracking-widest">Độ trong suốt</span>
                             <span className="text-sm font-black text-stone-900 dark:text-white">{glassOpacity}<span className="text-[10px] opacity-40 ml-0.5">%</span></span>
                          </div>
                          <div className="relative group/slider h-6 flex items-center">
                            <input 
                        ...
                        <div className="flex items-center justify-between p-4 bg-stone-50 dark:bg-white/5 rounded-2xl">
                           <div className="space-y-0.5">
                              <span className="text-xs font-bold text-stone-900 dark:text-stone-100">Kết cấu mặt kính (Texture)</span>
                              <p className="text-[10px] text-stone-500">Thêm hiệu ứng nhiễu hạt nhẹ tạo cảm giác chất liệu thật.</p>
                           </div>
                           <button 
                ...
                <CardHeader className="py-8 px-10 border-b border-stone-100 dark:border-white/5 bg-stone-50/50 dark:bg-white/2">
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <h2 className="text-xl font-black tracking-tighter text-stone-900 dark:text-white uppercase px-1">
                      {isSearching ? `Kết quả tìm kiếm cho "${searchQuery}"` : tabs.find(t => t.id === activeTab)?.label}
                    </h2>
                    {activeTab === 'academic' && !isSearching && (
                      <Button onClick={handleAddYear} className="rounded-2xl bg-stone-900 dark:bg-amber-600 text-white font-bold h-10 px-4 flex items-center gap-2">
                  ...
                  {activeTab === 'general' || activeTab === 'finance' ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-8">
                       {filteredSettings.map((setting) => (
                        <div key={setting.id} className="group flex flex-col gap-2">
                          ...
                          <p className="px-5 text-[9px] font-mono text-stone-400/50 lowercase italic tracking-tight">{setting.setting_key}</p>
                        </div>
                      ))}
                      {filteredSettings.length === 0 && (
                        <div className="md:col-span-2 py-20 text-center space-y-6">
                           ...
                           <p className="text-stone-400 font-medium italic">Không tìm thấy cài đặt nào trong danh mục này</p>
                        </div>
                      )}
                    </div>
                  ) : activeTab === 'academic' ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      {academicYears.map((year) => (
                        ...
                        >
                           {year.is_current && <div className="absolute top-0 right-0 w-32 h-32 bg-amber-500/10 blur-3xl rounded-full translate-x-1/2 -translate-y-1/2" />}
                           <div className="flex justify-between items-start mb-6">
                             <div className="p-4 bg-white dark:bg-stone-900 rounded-[22px] shadow-sm group-hover:bg-amber-500 transition-colors group-hover:text-white"><Icons.Calendar className="w-6 h-6" /></div>
                             <div className="flex flex-col items-end">
                               {year.is_current && <span className="px-3.5 py-1.5 bg-amber-500/10 text-amber-600 text-[10px] font-black uppercase tracking-widest rounded-full border border-amber-500/20">Active Now</span>}
                               <span className="text-[11px] font-black text-stone-400 mt-3 opacity-40 group-hover:opacity-100 transition-opacity tracking-widest">{year.id.split('-')[0]}</span>
                             </div>
                           </div>
                           <h3 className="text-2xl font-black text-stone-950 dark:text-white tracking-tight leading-none mb-2">{year.name}</h3>
                           <p className="text-sm text-stone-500 font-medium opacity-60">
                             {new Date(year.start_date).toLocaleDateString()} &mdash; {new Date(year.end_date).toLocaleDateString()}
                           </p>
                    ...
                    <div className="space-y-10">
                       <section className="space-y-6">
                          <h3 className="text-[10px] font-black text-stone-400 uppercase tracking-widest px-1">Phiên hoạt động</h3>
                          <div className="grid grid-cols-1 gap-4">
                             {[
                               { device: 'MacBook Pro 14"', browser: 'Chrome', ip: '192.168.1.1', location: 'TP. Hồ Chí Minh', current: true },
                               { device: 'iPhone 15 Pro', browser: 'Safari', ip: '112.45.2.1', location: 'TP. Hồ Chí Minh', current: false },
                               { device: 'Windows PC', browser: 'Edge', ip: '14.161.5.2', location: 'Hà Nội', current: false }
                             ].map((session, i) => (
                               <div key={i} className="flex items-center justify-between p-6 bg-white dark:bg-stone-900 rounded-3xl border border-stone-100 dark:border-white/5 group hover:border-amber-500/30 transition-all">
                                  <div className="flex items-center gap-4">
                                     ...
                                     <div>
                                        <div className="flex items-center gap-2">
                                           <span className="font-bold text-stone-900 dark:text-stone-100 text-[15px]">{session.device}</span>
                                           {session.current && <span className="text-[9px] font-black uppercase text-amber-500 px-2 py-0.5 bg-amber-500/10 rounded-full">Hiện tại</span>}
                                        </div>
                       ...
                       <section className="space-y-6">
                          <h3 className="text-[10px] font-black text-stone-400 uppercase tracking-widest px-1">Nhật ký hoạt động</h3>
                          <div className="space-y-1">
                             {[
                               { action: 'Đã thay đổi màu chủ đạo', date: '10 phút trước', category: 'Giao diện' },
                               { action: 'Đã cập nhật thang điểm học kỳ', date: '2 giờ trước', category: 'Thang điểm' },
                               { action: 'Đã thay đổi trạng thái năm học 2024-2025', date: 'Hôm qua', category: 'Năm học' },
                               { action: 'Đã đăng nhập từ thiết bị mới', date: '2 ngày trước', category: 'Bảo mật' }
                             ].map((log, i) => (
                               <div key={i} className="flex items-center justify-between p-4 hover:bg-stone-50 dark:hover:bg-white/2 rounded-2xl transition-all group">
                                  <div className="flex items-center gap-3">
                                     <div className="w-1.5 h-1.5 rounded-full bg-stone-300 dark:bg-stone-700 group-hover:bg-amber-500 transition-colors" />
                                     <div>
                                        <p className="text-sm font-bold text-stone-800 dark:text-stone-200">{log.action}</p>
                                        <div className="flex items-center gap-2 mt-0.5">
                                           <span className="text-[10px] font-medium text-stone-500 uppercase tracking-wider">{log.category}</span>
                                           <span className="text-[10px] text-stone-400">&bull; {log.date}</span>
                                        </div>
                                     </div>
                                  </div>
                    ...
                    <div className="space-y-10">
                       <section className="grid grid-cols-1 md:grid-cols-3 gap-6">
                          {[
                            ...
                            { label: 'Push Notifications', icon: Icons.Notifications, active: true },
                          ].map((channel, i) => (
                            <div key={i} className="p-6 bg-white dark:bg-stone-900 rounded-3xl border border-stone-100 dark:border-white/5 space-y-4">
                               <div className="flex justify-between items-start">
                                  ...
                                  <button className={cn(
                                    ...
                                    channel.active ? "bg-amber-500" : "bg-stone-200 dark:bg-stone-800"
                                  )}>
                                     <div className={cn("w-3 h-3 bg-white rounded-full transition-all", channel.active ? "translate-x-5" : "translate-x-0")} />
                                  </button>
                               </div>
                               <span className="block font-bold text-sm text-stone-800 dark:text-stone-200">{channel.label}</span>
                            </div>
                          ))}
                       </section>

                       <section className="space-y-4">
                          <h3 className="text-[10px] font-black text-stone-400 uppercase tracking-widest px-1">Sự kiện kích hoạt</h3>
                          <div className="bg-stone-50/50 dark:bg-white/2 rounded-[32px] border border-stone-100 dark:border-white/5 overflow-hidden">
                             {[
                               { label: 'Điểm số mới', description: 'Thông báo khi hể thống cập nhật điểm cho học sinh', push: true, email: true },
                               { label: 'Yêu cầu thanh toán', description: 'Thông báo khi có hóa đơn học phí mới cần thanh toán', push: true, email: true },
                               { label: 'Thay đổi thời khóa biểu', description: 'Thông báo khi lịch học hoặc giáo viên thay đổi', push: true, email: false },
                               { label: 'Cập nhật hệ thống', description: 'Thông báo về các bảo trì hoặc tính năng mới', push: false, email: true },
                             ].map((event, i) => (
                               <div key={i} className="flex items-center justify-between p-6 border-b border-stone-100 dark:border-white/5 last:border-0 hover:bg-white dark:hover:bg-white/2 transition-all">
                                  <div className="space-y-1">
                                     <span className="font-bold text-stone-900 dark:text-stone-100 text-[15px]">{event.label}</span>
                                     <p className="text-xs text-stone-500 font-medium">{event.description}</p>
                                  </div>
                                  <div className="flex items-center gap-6">
                                     <div className="flex flex-col items-center gap-2">
                                        <span className="text-[8px] font-black text-stone-400 uppercase tracking-widest">Push</span>
                                        <input type="checkbox" checked={event.push} readOnly className="w-4 h-4 rounded border-stone-300 text-amber-500 focus:ring-amber-500" />
                                     </div>
                                     <div className="flex flex-col items-center gap-2">
                                        <span className="text-[8px] font-black text-stone-400 uppercase tracking-widest">Email</span>
                                        <input type="checkbox" checked={event.email} readOnly className="w-4 h-4 rounded border-stone-300 text-amber-500 focus:ring-amber-500" />
                                     </div>
                                  </div>
                               </div>
                             ))}
