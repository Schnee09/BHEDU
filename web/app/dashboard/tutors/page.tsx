...
function TutorsContent() {
  ...
  return (
    <div className="p-6 md:p-10 max-w-[1600px] mx-auto space-y-8 relative z-10">
      ...
      {/* Hidden trap to prevent browser credentials autofill */}
      ...
      <div className="mb-6">
        <div className="relative max-w-md">
          ...
          <input
            type="text"
            name="tutors_search_query"
            id="tutors_search_query"
            autoComplete="off"
            data-lpignore="true"
            ...
            className="w-full pl-10 pr-10 py-2 border rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 bg-white dark:bg-stone-900 text-stone-900 dark:text-stone-100"
          />
          {searchQuery && (
            <button
              type="button"
              onClick={() => setSearchQuery('')}
              className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 p-0.5 rounded-full cursor-pointer transition-colors"
              title="Xóa tìm kiếm"
            >
              <X className="w-4 h-4" />
            </button>
      ...
      {loading ? (
        ...
        <div className="text-center py-12 bg-gray-50 dark:bg-stone-900/50 rounded-2xl border border-stone-200/60 dark:border-stone-800">
          <GraduationCap className="w-16 h-16 text-gray-300 dark:text-stone-700 mx-auto mb-4" />
          <p className="text-gray-600 dark:text-stone-400 mb-4 font-medium">
            {searchQuery
              ? `Không tìm thấy gia sư nào khớp với từ khóa "${searchQuery}"`
          ...
          {searchQuery ? (
            <button
              type="button"
              onClick={() => setSearchQuery('')}
              className="px-4 py-2 bg-stone-200 hover:bg-stone-300 dark:bg-stone-800 dark:hover:bg-stone-700 text-stone-800 dark:text-stone-200 rounded-xl text-xs font-bold inline-flex items-center gap-1.5 cursor-pointer transition-colors"
            >
              <X className="w-4 h-4" /> Xóa bộ lọc tìm kiếm
            </button>
          ) : (
            canManage && (
              <button
