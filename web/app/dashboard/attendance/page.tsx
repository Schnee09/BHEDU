    ...
    return (
      <div className="min-h-screen bg-transparent py-8 px-4 sm:px-6 lg:px-10">
        <div className="max-w-[1600px] mx-auto space-y-8">
          <div className="h-40 w-full glass-premium rounded-[40px] animate-pulse" />
    ...
    return (
      <div className="min-h-screen bg-transparent py-8 px-4 sm:px-6 lg:px-10">
        <div className="max-w-[1600px] mx-auto space-y-8 relative z-10">
          ...
          {/* Stats Grid */}
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            <div className="glass-premium rounded-[32px] border border-white/20 shadow-xl p-6 hover-up transition-all group overflow-hidden relative">
              <div className="absolute top-0 right-0 w-24 h-24 bg-stone-500/5 blur-3xl rounded-full translate-x-12 -translate-y-12" />
              <div className="relative z-10 flex flex-col justify-between h-full">
                <div className="flex items-center justify-between mb-4">
                  <p className="text-[10px] font-black uppercase tracking-widest text-stone-400">
                    {t('attendance.dashboard.totalDays')}
                  </p>
                  <CalendarDaysIcon className="w-5 h-5 text-stone-300" />
                </div>
                <p className="text-4xl font-black text-stone-900 tracking-tighter">{stats.totalDays}</p>
              </div>
            </div>

            <div className="glass-premium rounded-[32px] border border-emerald-500/10 shadow-xl p-6 hover-up transition-all group overflow-hidden relative">
              <div className="absolute top-0 right-0 w-24 h-24 bg-emerald-500/10 blur-3xl rounded-full translate-x-12 -translate-y-12" />
              <div className="relative z-10 flex flex-col justify-between h-full">
                <div className="flex items-center justify-between mb-4">
                ...
                <p className="text-4xl font-black text-emerald-600 tracking-tighter">{stats.presentDays}</p>
              </div>
            </div>

            <div className="glass-premium rounded-[32px] border border-red-500/10 shadow-xl p-6 hover-up transition-all group overflow-hidden relative">
              <div className="absolute top-0 right-0 w-24 h-24 bg-red-500/10 blur-3xl rounded-full translate-x-12 -translate-y-12" />
              <div className="relative z-10 flex flex-col justify-between h-full">
                <div className="flex items-center justify-between mb-4">
                ...
                <p className="text-4xl font-black text-red-600 tracking-tighter">{stats.absentDays}</p>
              </div>
            </div>

            <div className="glass-premium rounded-[32px] border border-amber-500/10 shadow-xl p-6 hover-up transition-all group overflow-hidden relative">
              <div className="absolute top-0 right-0 w-32 h-32 bg-amber-500/10 blur-3xl rounded-full translate-x-12 -translate-y-12" />
              <div className="relative z-10 flex flex-col justify-between h-full">
                <div className="flex items-center justify-between mb-4">
                ...
                <p className="text-4xl font-black text-amber-600 tracking-tighter">{stats.attendanceRate}%</p>
                <div className="mt-2 h-1.5 bg-amber-100 rounded-full overflow-hidden">
                  <div className="h-full bg-amber-500" style={{ width: `${stats.attendanceRate}%` }} />
                </div>
              </div>
            </div>
          </div>
              ...
              <div className="glass-premium rounded-[32px] border border-white/20 p-8 shadow-xl animate-fade-in">
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-8">
                  <div className="space-y-2">
                    <label className="text-[11px] font-black uppercase tracking-widest text-stone-500 ml-1">{t('common.date')}</label>
                    <Input
                      type="date"
                  ...
                  <div className="space-y-2">
                    <label className="text-[11px] font-black uppercase tracking-widest text-stone-500 ml-1">{t('common.status')}</label>
                    <Select
                    ...
                    <div className="space-y-4 p-4">
                      {attendanceRecords.map((record) => (
                        <MobileCard
                          ...
                          subtitle={record.date ? format(new Date(record.date), 'EEEE, dd/MM/yyyy', { locale: dateLocale }) : '---'}
                          status={{
                            label: record.status === 'present' ? t('attendance.present') : t('attendance.absent'),
                    ...
                    <table className="w-full">
                      <thead>
                        <tr className="bg-stone-50/50">
                          <th className="px-8 py-5 text-left text-[10px] font-black uppercase tracking-widest text-stone-400">{t('common.date')}</th>
                          <th className="px-8 py-5 text-left text-[10px] font-black uppercase tracking-widest text-stone-400">{t('students.class')}</th>
                          <th className="px-8 py-5 text-left text-[10px] font-black uppercase tracking-widest text-stone-400">{t('grades.subject')}</th>
                          <th className="px-8 py-5 text-center text-[10px] font-black uppercase tracking-widest text-stone-400">{t('common.status')}</th>
                          <th className="px-8 py-5 text-left text-[10px] font-black uppercase tracking-widest text-stone-400">{t('common.remarks')}</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-stone-50">
                        {attendanceRecords.map((record) => (
                          <tr key={record.id} className="hover:bg-emerald-50/30 transition-colors group">
                            ...
                            <td className="px-8 py-6 text-sm font-bold text-stone-900">{record.className}</td>
                            <td className="px-8 py-6 text-sm font-bold text-stone-600">{record.subjectName || '-'}</td>
                            <td className="px-8 py-6 text-center">
                              <Badge variant={record.status === 'present' ? 'success' : 'danger'} className="rounded-full px-4 py-1 font-black uppercase tracking-widest text-[9px]">
                                {record.status === 'present' ? t('attendance.present') : t('attendance.absent')}
                              </Badge>
                            </td>
                            <td className="px-8 py-6 text-sm text-stone-400 italic">{record.remarks || '-'}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
      ...
      <div className="max-w-[1600px] mx-auto space-y-10 relative z-10">
            ...
            <Link key={section.href} href={section.href} className="group">
              <Card className="h-full glass-premium hover-up transition-all border border-white/20 shadow-xl rounded-[32px] overflow-hidden relative">
                ...
                <CardBody className="flex flex-col items-center text-center p-10 relative">
                  <div className={`p-5 rounded-[24px] mb-6 transition-all group-hover:scale-110 border ${section.color}`}>
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {recentClasses.map(cls => (
                <div key={cls.id} className="glass-premium p-6 rounded-[28px] border border-white/20 shadow-lg flex items-center justify-between hover-up transition-all group">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <p className="font-black text-stone-900 tracking-tight uppercase text-sm">{cls.name}</p>
                      <p className="text-xs font-bold text-stone-400 uppercase tracking-widest">{cls.subject_name || 'Instruction'}</p>
                    </div>
                  </div>
                  <Link
        ...
        <div className="glass-premium bg-stone-900 rounded-[40px] border-none shadow-2xl overflow-hidden relative group">
          ...
          <CardBody className="flex flex-col md:flex-row items-center md:items-start gap-8 p-12 relative z-10 text-white">
            ...
            <div className="space-y-6">
              <h3 className="text-2xl font-black tracking-tight uppercase italic text-emerald-400">{t('attendance.dashboard.tipTitle')}</h3>
