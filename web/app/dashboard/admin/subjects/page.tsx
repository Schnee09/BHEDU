  ...
  const toggleActive = async (subject: Subject) => {
    try {
      ...
      if (response.ok) {
        ...
        toast.success(
          'Thành công',
          `Đã ${!subject.is_active ? 'kích hoạt' : 'vô hiệu hóa'} môn học`
        );
      } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        toast.success(
          'Thành công',
          modalMode === 'create' ? 'Đã thêm môn học mới' : 'Đã cập nhật môn học'
        );
        setShowModal(false);
        fetchSubjects();
      } else {
      ...
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-500">
            Bạn không có quyền quản lý môn học. Vui lòng liên hệ quản trị viên.
          </p>
        </div>
      </div>
    );
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                  {loading ? (
                    ...
                    filteredSubjects.map((subject) => (
                      <tr
                        key={subject.id}
                        className="group hover:bg-gray-50/50 dark:hover:bg-gray-700/20 transition-all"
                      >
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-4">
                            <div
                              className={`p-2.5 rounded-xl transition-colors ${subject.is_active ? 'bg-emerald-50 text-emerald-600 dark:bg-emerald-900/30' : 'bg-gray-50 text-gray-400 dark:bg-gray-700'}`}
                            >
                        ...
                        <td className="px-6 py-4">
                          <div className="flex justify-center">
                            ...
                            >
                              <span
                                className={`w-1.5 h-1.5 rounded-full ${subject.is_active ? 'bg-emerald-500 animate-pulse' : 'bg-gray-400'}`}
                              />
            ...
            <div className="p-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {loading ? (
                Array.from({ length: 6 }).map((_, i) => (
                  <div
                    key={i}
                    className="h-40 bg-gray-50 dark:bg-gray-700/50 rounded-2xl animate-pulse"
                  />
                ))
              ) : filteredSubjects.length === 0 ? (
                ...
                filteredSubjects.map((subject) => (
                  <div
                    key={subject.id}
                    className="group relative bg-gray-50/50 dark:bg-gray-700/20 border border-gray-100 dark:border-gray-700/50 rounded-2xl p-6 transition-all hover:shadow-lg hover:border-blue-200 dark:hover:border-blue-900/50"
                  >
                    <div className="flex items-start justify-between mb-4">
                      <div
                        className={`p-3 rounded-xl ${subject.is_active ? 'bg-emerald-100 text-emerald-600 dark:bg-emerald-900/40 dark:text-emerald-400' : 'bg-gray-200 text-gray-400 dark:bg-gray-700'}`}
                      >
