                ...
                {loading ? (
                  ...
                  <tr>
                    <td colSpan={5} className="px-6 py-20 text-center">
                      <p className="text-xs font-bold text-stone-500 uppercase tracking-[0.3em]">
                        Không tìm thấy dữ liệu truy vấn
                      </p>
                    </td>
                  </tr>
                ) : (
                  logs.map((log) => (
                    <tr key={log.id} className="hover:bg-white/5 transition-colors group">
                      ...
                      <td className="px-6 py-5">
                        <span
                          className={`px-3 py-1 rounded-sharp text-[10px] font-bold uppercase tracking-widest ${actionColors[log.action] || 'bg-stone-100 dark:bg-stone-800 text-stone-500'}`}
                        >
          ...
          {totalPages > 1 && (
            <div className="px-8 py-6 border-t border-stone-100 dark:border-stone-800 flex items-center justify-between">
              <p className="text-[10px] font-bold text-stone-500 uppercase tracking-widest">
                BẢN GHI {(page - 1) * pageSize + 1} - {Math.min(page * pageSize, totalCount)} /{' '}
                {totalCount}
              </p>
