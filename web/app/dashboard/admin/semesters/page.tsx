...
export default function SemesterManagementPage() {
  ...
  const handleSetActive = async (semesterId: string) => {
    try {
      const response = await apiFetch(`/api/admin/semesters/${semesterId}/activate`, {
        method: 'POST',
      });
      if (response.ok) {
  ...
  const handleDelete = async () => {
    ...
    setSubmitting(true);
    try {
      const response = await apiFetch(`/api/admin/semesters/${semesterToDelete.id}`, {
        method: 'DELETE',
      });
      if (response.ok) {
      ...
      <div className="min-h-screen flex items-center justify-center p-4">
        <Card variant="glass" className="max-w-md text-center p-8">
          ...
          <p className="text-muted mb-6">
            Bạn không có quyền quản lý học kỳ. Vui lòng liên hệ quản trị viên.
          </p>
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 bg-white dark:bg-gray-800/80 p-8 rounded-3xl border border-gray-100 dark:border-white/5 shadow-premium backdrop-blur-md">
          <div>
            ...
            <p className="text-muted mt-2 max-w-lg">
              Cấu hình các giai đoạn học tập, thời gian bắt đầu và kết thúc của mỗi học kỳ trong năm
              học.
            </p>
          </div>
          <Button
        ...
        {loading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {Array.from({ length: 6 }).map((_, i) => (
              <div
                key={i}
                className="h-64 bg-white/50 dark:bg-white/5 rounded-3xl animate-pulse border border-gray-100 dark:border-white/5"
              />
            ))}
          </div>
        ) : semesters.length === 0 ? (
      ...
      >
        <div className="text-center p-4">
          ...
          <p className="text-muted">
            Hành động này không thể hoàn tác. Học kỳ <strong>{semesterToDelete?.name}</strong> sẽ bị
