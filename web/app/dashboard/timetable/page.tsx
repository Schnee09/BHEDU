  ...
  const fetchClassSlots = async () => {
    ...
    try {
      ...
      const response = await apiFetch(
        `/api/timetable?class_id=${selectedClass}&week_start_date=${weekStartStr}`
      );
      const data = await response.json();
      // Unwrap V2 response if needed
      const slotsData = data.data?.slots || data.slots || [];
  ...
  const saveSlot = async () => {
    ...
    try {
      ...
      if (slotId && formData.weekly_note) {
        ...
        await apiFetch(
          `/api/timetable/weekly-notes?slot_id=${slotId}&week_start_date=${weekStartStr}`,
          {
            method: 'DELETE',
          }
        );
        ...
        <div className="flex flex-col xl:flex-row xl:items-end justify-between gap-6 bg-white dark:bg-gray-800/80 p-8 rounded-[32px] border border-gray-100 dark:border-white/5 shadow-premium backdrop-blur-md">
          <div>
            ...
            <p className="text-muted mt-2 max-w-2xl font-medium">
              {weekDatesDesktop[0]?.toLocaleDateString('vi-VN')} -{' '}
              {weekDatesDesktop[6]?.toLocaleDateString('vi-VN')}
            </p>
          </div>
            ...
            <div className="flex items-center gap-3 bg-gray-100 dark:bg-white/5 p-1.5 rounded-2xl">
              <div className="flex items-center gap-3 px-3 border-r border-gray-200 dark:border-gray-700 mr-1 pr-4">
                <Link
                  ...
                  className="flex items-center gap-1.5 text-[11px] font-black uppercase tracking-widest text-stone-500 hover:text-amber-600 dark:hover:text-amber-400 transition-colors"
                >
                ...
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                    {slots.filter((s) => !s.room || s.room === 'Linh hoạt').length === 0 ? (
                      ...
                      slots
                        .filter((s) => !s.room || s.room === 'Linh hoạt')
                        .map((slot) => (
                          <tr
                            key={slot.id}
                          ...
                          >
                            <td className="p-5">
                              ...
                              <div className="text-[10px] text-amber-500 font-black uppercase tracking-widest">
                                {weekDatesDesktop[slot.day_of_week]?.toLocaleDateString('vi-VN')}
                              </div>
                            </td>
                            <td className="p-5">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] font-black text-stone-400 dark:text-stone-500 uppercase tracking-[0.2em]">
                    ...
                    <span className="text-stone-900 dark:text-stone-100">
                      {slots.filter((s) => !s.room || s.room === 'Linh hoạt').length}
                    </span>{' '}
                    buổi học kèm
                  </span>
                </div>
                      ...
                      {DAYS.map((day, i) => (
                        ...
                        >
                          ...
                          <div className="text-[10px] text-amber-500 font-black uppercase tracking-widest mt-0.5">
                            {weekDatesDesktop[i]?.toLocaleDateString('vi-VN', {
                              day: '2-digit',
                              month: '2-digit',
                            })}
                          </div>
                        ...
                        tutors.map((tutor, tutorIdx) => (
                          <tr key={`${session.id}-${tutor.id}`} className="group transition-colors">
                            {tutorIdx === 0 && (
                              ...
                              >
                                <div className="font-black text-amber-500 text-lg leading-tight">
                                  {session.label}
                                </div>
                    ...
                    currentCampus?.rooms.map((room, roomIdx) => (
                      <tr key={`${session.id}-${room}`} className="group transition-colors">
                        ...
                        {DAYS.map((_, dayIndex) => {
                          const isAvailable = session.days.includes(dayIndex);
                          const slot = isAvailable
                            ? getSlotForRoomCell(room, dayIndex, session.start)
                            : null;
                            ...
                            >
                              {!isAvailable ? (
                                ...
                                >
                                  ...
                                  {slot.subject && (
                                    <div className="text-amber-600 dark:text-amber-500 text-[10px] font-black uppercase tracking-wider mb-2">
                                      {slot.subject.name}
                                    </div>
                                  )}
                                  <div className="flex items-center gap-3 mt-auto">
                                    ...
                                    {slot.has_weekly_note && (
                                      <div
                                        className="w-2 h-2 rounded-full bg-amber-500 shadow-[0_0_8px_rgba(245,166,35,0.6)]"
                    ...
                    {ALL_SESSIONS.map((session) => (
                      <tr key={session.id} className="group transition-colors">
                        ...
                        {DAYS.map((_, dayIndex) => {
                          const isAvailable = session.days.includes(dayIndex);
                          const slot = isAvailable
                            ? getSlotForClassCell(dayIndex, session.start)
                            : null;
                          return (
                            <td
                              key={dayIndex}
                    ...
                    {ALL_SESSIONS.map((session) => (
                      <tr key={session.id} className="group transition-colors">
                        ...
                        {DAYS.map((_, dayIndex) => {
                          const isAvailable = session.days.includes(dayIndex);
                          const slot = isAvailable
                            ? getSlotForTeacherCell(selectedTeacher, dayIndex, session.start)
                            : null;
                          return (
                            <td
                              key={dayIndex}
