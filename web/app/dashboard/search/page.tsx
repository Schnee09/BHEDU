...
function SearchContent() {
    ...
    const performSearch = async () => {
      ...
      try {
        ...
        const features = [
          {
            ...
            keywords: ['schedule', 'lich hoc', 'thoi khoa bieu'],
          },
          {
            name: 'Điểm danh',
          ...
          {
            name: 'Học phí & Tài chính',
            href: '/dashboard/finance',
            keywords: ['finance', 'hoc phi', 'tien hoc'],
          },
          ...
          { name: 'Lớp học', href: '/dashboard/classes', keywords: ['classes', 'lop hoc'] },
          {
            ...
            keywords: ['profile', 'ca nhan', 'ho so'],
          },
          {
        ...
        const studentsRes = await apiFetch(
          `/api/students?search=${encodeURIComponent(query)}&limit=10`
        );
        if (studentsRes.ok) {
          ...
          students.forEach((s: any) => {
            searchResults.push({
              type: 'student',
              id: s.id,
              title: s.full_name || formatVietnameseName(s.first_name, s.last_name),
        ...
        const classesRes = await apiFetch(
          `/api/classes?search=${encodeURIComponent(query)}&limit=10`
        );
        if (classesRes.ok) {
  ...
  const getTypeBadge = (type: string) => {
    switch (type) {
      case 'student':
        return (
          <Badge variant="info" className="text-[10px] font-black uppercase">
            Học sinh
          </Badge>
        );
      case 'class':
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8 md:py-12 space-y-10">
        ...
        <div className="space-y-2">
          ...
          {query && (
            <p className="text-xs font-black text-stone-400 dark:text-stone-500 uppercase tracking-[0.2em] pl-4">
              Kết quả cho:{' '}
              <span className="text-stone-900 dark:text-stone-100 italic">"{query}"</span>
            </p>
          )}
        </div>
      </div>
      ...
      <Card
        padding="p-2"
        className="bg-stone-50/50 dark:bg-white/5 border-stone-200/50 dark:border-white/10 shadow-sm"
      >
        <form
          action="/dashboard/search"
          method="GET"
          className="p-2 flex flex-col sm:flex-row gap-3"
        >
          ...
          <Button
            type="submit"
            variant="primary"
            className="px-8 py-3.5 font-black uppercase tracking-widest text-xs h-auto shadow-amber-glow"
          >
            Tìm kiếm ngay
          </Button>
        </form>
            ...
            {results.length === 0 ? (
              <div className="py-20 text-center space-y-4 opacity-50">
                ...
                <div className="space-y-1">
                  <p className="text-xl font-serif font-black text-stone-900 dark:text-white uppercase tracking-tight">
                    Không tìm thấy kết quả
                  </p>
                  <p className="text-[10px] font-black text-stone-400 uppercase tracking-widest">
                    Vui lòng thử từ khóa khác đềEcó kết quả tốt hơn
                  </p>
                </div>
              </div>
            ) : (
              <div className="space-y-4">
                <div className="flex items-center justify-between px-2">
                  <p className="text-[10px] font-black text-stone-400 uppercase tracking-widest pl-3 border-l-4 border-emerald-500">
                    PHÁT HIềE {results.length} KẾT QUẢ ĐỐI CHIẾU
                  </p>
                </div>
                <div className="grid grid-cols-1 gap-3">
                  {results.map((result) => (
                    <Link key={`${result.type}-${result.id}`} href={result.href} className="group">
                      <Card
                        padding="p-4"
                        className="hover:border-emerald-500/40 hover:shadow-md transition-all duration-300 hover:-translate-y-1 overflow-hidden relative"
                      >
          ...
          <div className="py-24 text-center space-y-6">
            ...
            <div className="space-y-2">
              ...
              <p className="text-[10px] font-black text-stone-400 dark:text-stone-500 uppercase tracking-[0.3em]">
                Nhập từ khóa đềEbắt đầu truy vấn hềEthống
              </p>
            </div>
          </div>
        )}
