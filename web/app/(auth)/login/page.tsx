...
export default function LoginPage() {
  return (
    <GuestGuard>
      <Suspense
        fallback={
          <div className="min-h-screen flex items-center justify-center bg-stone-100 dark:bg-[#0E0D0B]">
...
function LoginContent() {
  ...
  useEffect(() => {
    ...
    if (err === 'account_not_found') {
      setError(
        `Tài khoản Google (${email || 'này'}) chưa được liên kết với bất kỳ hồ sơ học vụ nào tại BH-EDU. Vui lòng liên hệ trung tâm để được cấp hồ sơ hoặc đăng nhập bằng Mã UID / Số điện thoại.`
      );
    } else if (err === 'account_deactivated') {
      setError('Tài khoản này đang bị tạm khóa. Vui lòng liên hệ trung tâm để được hỗ trợ.');
    } else if (err === 'oauth_exchange_failed' || err === 'auth_failed') {
  ...
  const handleRedirect = (role: string | null | undefined) => {
    switch (role) {
      ...
      case 'teacher':
        ...
        break;
      case 'tutor':
        router.replace('/dashboard/tutor/students');
        break;
      case 'owner':
      case 'super_admin':
      case 'admin':
        ...
        <div className="grid grid-cols-2 gap-1.5 p-1 rounded-2xl bg-stone-100 dark:bg-[#1C1A16] border border-stone-200 dark:border-stone-800">
          <button
            ...
            className={cn(
              ...
              authMethod === 'standard'
                ? 'bg-white dark:bg-[#25221D] text-amber-600 dark:text-amber-400 shadow-xs'
                : 'text-stone-500 hover:text-stone-800 dark:text-stone-400 dark:hover:text-stone-200'
            )}
          >
            <KeyRound className="w-3.5 h-3.5" />
            <span>Mã UID / Mật khẩu</span>
          </button>
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="Mã HS / Mã GV / SĐT / Email..."
              ...
              <p className="text-[10px] text-stone-400 dark:text-stone-500">
                Nhập Mã học sinh (VD: HS2026...), Mã giáo viên, SĐT hoặc Email.
              </p>
            </div>
