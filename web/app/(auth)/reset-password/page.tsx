...
export default function ResetPasswordPage() {
  ...
  useEffect(() => {
    const checkSession = async () => {
      ...
      if (!data.session) {
        toast.error('Liên kết đặt lại mật khẩu của bạn đã hết hạn hoặc không hợp lệ.');
        ...
        <div className="p-10 md:p-12 space-y-8">
          {verifying ? (
            <div className="py-12 text-center space-y-4">
              <Icons.History className="w-10 h-10 animate-spin mx-auto text-amber-500" />
              <p className="text-stone-500 font-bold uppercase tracking-widest text-[10px]">
                Đang xác thực yêu cầu...
              </p>
            </div>
          ) : (
            <>
              ...
              {success ? (
                <div className="py-12 text-center space-y-6 animate-in fade-in zoom-in duration-500">
                  ...
                  <div className="space-y-2">
                    <p className="text-xl font-bold text-stone-900 dark:text-white uppercase tracking-tight">
                      Cập nhật thành công!
                    </p>
                    <p className="text-sm text-stone-500">
                      Bạn sẽ được chuyển hướng sang trang đăng nhập trong giây lát...
                    </p>
                  </div>
                </div>
              ) : (
