...
const _updateCurriculumStandardSchema = createCurriculumStandardSchema
  .partial();

// GET /api/curriculum-standards - List curriculum standards with filters
export async function GET(request: NextRequest) {
  try {
    // Check view permission. Students, Parents, Teachers, Staff, Admin all likely have this.
    // Core defines "curriculum.view" for Student. Higher roles inherit or have manage.
    // Let's implement the check.
    const { authorized } = await getAuthContext(request, "curriculum.view");
    if (!authorized) {
