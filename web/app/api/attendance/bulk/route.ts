...
export async function POST(req: NextRequest) {
  try {
    ...
    // Use service client for consistent data access (matching the GET endpoint)
    ...
    if (!classId || !date || !records || !Array.isArray(records)) {
      return NextResponse.json(
        {
          error:
            "Invalid request: classId, date, and records array are required",
        },
        { status: 400 },
      );
    }
    ...
    for (const record of records) {
      try {
        ...
        // Only use columns that exist in the attendance table
        ...
        logger.error(
          `Unexpected error marking attendance for student ${record.studentId}`,
          error,
        );
        results.errors.push({
          studentId: record.studentId,
