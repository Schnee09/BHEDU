...
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    if (gradesError) {
      ...
      return NextResponse.json({ error: 'Failed to fetch grade history' }, { status: 500 });
    }
    ...
    const trendData = Object.entries(gradesByMonth)
      ...
      .map(([month, data]) => {
        const avg =
          data.grades.length > 0 ? data.grades.reduce((s, g) => s + g, 0) / data.grades.length : 0;
    ...
    const subjectData = Object.entries(subjectAverages)
      .map(([id, data]) => ({
        ...
        average:
          Math.round((data.grades.reduce((s, g) => s + g, 0) / data.grades.length) * 100) / 100,
        count: data.grades.length,
      }))
...
function formatMonthLabel(monthKey: string): string {
  ...
  const monthIdx = month ? parseInt(month) : 0;
  const monthName = months[monthIdx] ?? 'Th?';
