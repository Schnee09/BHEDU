...
// POST /api/grades
export const POST = createApiHandler({ permission: 'grades.manage' }, async ({ body }) => {
  const supabase = createServiceClient();
  ...
  if ('grades' in validatedData) {
    ...
    const { data: insertedData, error } = await supabase.from('grades').insert(gradeRows).select();
