    ...
    if (role && role !== existingUser.role) {
      await supabase
        ...
        .insert({
          ...
          action: "role_changed",
          description:
            `Role changed from ${existingUser.role} to ${role} by ${authResult.userEmail}`,
          metadata: {
    ...
    if (permanent) {
      ...
      await supabase
        ...
        .insert({
          ...
          action: "user_deleted",
          description:
            `User ${profile.full_name} (${profile.email}) permanently deleted by admin: ${authResult.userEmail}`,
