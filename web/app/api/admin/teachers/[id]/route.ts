 ...
 */
export const GET = createGetHandler({ permission: 'users.view' }, async ({ params, user }) => {
  ...
  async ({ params, user: actor }) => {
    ...
    if (activeClasses && activeClasses > 0) {
      return apiSuccess(
        {
          success: false,
          error: `Không thể vô hiệu hóa giáo viên đang có ${activeClasses} lớp học đang hoạt động.`,
        },
        { _status: 409 }
      );
    }
    ...
    await supabase.from('profiles').update({ is_active: false, status: 'inactive' }).eq('id', id);
