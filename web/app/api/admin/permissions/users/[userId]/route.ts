...
export async function GET(request: NextRequest, context: RouteContext) {
  try {
    ...
    try {
      ...
      if (error) {
        console.warn(
          "[Permissions API] user_permissions query error:",
          error.message,
          error.code,
        );
      } else {
    ...
    try {
      ...
      if (error) {
        console.warn(
          "[Permissions API] role_permissions query error:",
          error.message,
          error.code,
        );
      } else {
    ...
    console.log(
      "[Permissions API] Success - rolePerms:",
      rolePerms.length,
      "customPerms:",
      customPerms.length,
    );
...
export async function POST(request: NextRequest, context: RouteContext) {
  try {
    ...
    if (!permission_code) {
      return NextResponse.json({ error: "permission_code is required" }, {
        status: 400,
      });
    }
    ...
    // Grant permission
    console.log(
      "[Permissions API] Granting permission:",
      permission_code,
      "to user:",
      userId,
    );
    ...
    try {
      ...
      console.warn(
        "[Permissions API] Audit log error (non-fatal):",
        auditError,
      );
    }
