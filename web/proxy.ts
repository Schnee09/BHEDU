...
const PUBLIC_ROUTES = ['/', '/login', '/signup', '/forgot-password', '/reset-password', '/checkin'];

// Routes that require admin/staff access
const ADMIN_ROUTES = ['/dashboard/users', '/dashboard/admin', '/dashboard/settings'];

// Routes that require at least teacher access
const TEACHER_ROUTES = ['/dashboard/grades/entry', '/dashboard/attendance/mark'];
...
export async function proxy(request: NextRequest) {
  ...
  if (pathname.startsWith('/dashboard')) {
    ...
    if (ADMIN_ROUTES.some((route) => pathname.startsWith(route))) {
      if (!['admin', 'staff', 'super_admin', 'owner'].includes(userRole)) {
        const url = request.nextUrl.clone();
    ...
    if (TEACHER_ROUTES.some((route) => pathname.startsWith(route))) {
      if (!['admin', 'staff', 'teacher', 'super_admin', 'owner'].includes(userRole)) {
        const url = request.nextUrl.clone();
        url.pathname = '/dashboard';
        url.searchParams.set('error', 'unauthorized');
        return NextResponse.redirect(url);
    ...
    // Create new request headers with role and ID
    const requestHeaders = new Headers(request.headers);
    requestHeaders.set('x-user-role', userRole);
    requestHeaders.set('x-user-id', user.id);

    // Return new response with updated request headers
    return NextResponse.next({
      request: {
