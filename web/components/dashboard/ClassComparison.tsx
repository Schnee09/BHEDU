        ...
        {viewMode === 'table' ? (
            ...
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-stone-50 dark:divide-white/5">
                {sortedClasses.map((cls, index) => (
                  <tr
                    key={cls.classId}
                    className="group hover:bg-stone-50 dark:hover:bg-white/5 transition-all duration-300"
                  >
                    <td className="px-6 py-5">
                      <div
                        className={cn(
                          ...
                          index === 0
                            ? 'bg-amber-500/10 text-amber-600 shadow-accent-glow'
                            : index === 1
                    ...
                    <td className="px-4 py-5">
                      <span className="font-black text-stone-900 dark:text-white group-hover:text-emerald-600 transition-colors uppercase tracking-tight text-sm">
                        {cls.className}
                      </span>
                    </td>
                    ...
                    <td className="px-4 py-5 text-center">
                      <Badge
                        variant={getGPABadgeVariant(cls.averageGPA)}
                        className="font-black tracking-tighter tabular-nums px-3"
                      >
          ...
          <div className="p-8 w-full min-w-0">
            <ResponsiveContainer
              ...
              minWidth={0}
              minHeight={380}
              initialDimension={{ width: 500, height: 380 }}
            >
              <BarChart
              ...
              >
                ...
                <Bar dataKey="averageGPA" radius={[0, 6, 6, 0]} barSize={24}>
                  {sortedClasses.slice(0, 10).map((entry, index) => (
                    <Cell
                      key={`cell-${index}`}
                      fill={getGPAColor(entry.averageGPA)}
                      fillOpacity={0.8}
                    />
                  ))}
                </Bar>
