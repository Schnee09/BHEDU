...
export default function RankingWidget({ limit = 10, showAtRisk = true }: RankingWidgetProps) {
    const { data, loading, error, refetch } = useFetch<{ topStudents: StudentRanking[], atRiskStudents: StudentRanking[] }>(
    ...
    const getRankIcon = (rank: number) => {
        if (rank === 1) return <div className="w-8 h-8 rounded-full bg-amber-500/10 flex items-center justify-center shadow-accent-glow"><Trophy className="w-4 h-4 text-amber-500" /></div>;
        if (rank === 2) return <div className="w-8 h-8 rounded-full bg-slate-500/10 flex items-center justify-center"><Medal className="w-4 h-4 text-slate-400" /></div>;
        if (rank === 3) return <div className="w-8 h-8 rounded-full bg-orange-500/10 flex items-center justify-center"><Medal className="w-4 h-4 text-orange-600" /></div>;
        return <div className="w-8 h-8 rounded-full bg-stone-100 dark:bg-white/5 flex items-center justify-center text-[10px] font-black text-stone-400">#{rank}</div>;
    };
            ...
            <Card padding="p-0">
                <CardHeader className="flex items-center justify-between border-b border-stone-100 dark:border-white/5 bg-stone-50/30 dark:bg-white/5">
                    <div className="flex items-center gap-3">
                        ...
                        <h3 className="font-serif font-black text-lg text-stone-900 dark:text-white uppercase tracking-tight">Học sinh xuất sắc</h3>
                    </div>
                    <button
                        onClick={refetch}
                        className={cn(
            ...
            {showAtRisk && (
                <Card padding="p-0" className="border-red-500/20">
                    <CardHeader className="flex items-center gap-3 border-b border-stone-100 dark:border-white/5 bg-red-500/5">
                        ...
                        <h3 className="font-serif font-black text-lg text-stone-900 dark:text-white uppercase tracking-tight">Cần hỗ trợ</h3>
                    </CardHeader>
                            ...
                            atRiskStudents.map((student) => (
                                ...
                                >
                                    ...
                                    <div className="text-right">
                                        ...
                                        <span className="text-[9px] font-black text-red-500/60 uppercase tracking-tighter">Dưới 5.0</span>
                                    </div>
                                </Link>
                            ))
                        )}
