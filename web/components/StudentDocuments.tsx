...
export default function StudentDocuments({ studentId }: StudentDocumentsProps) {
  ...
  const getFileIcon = (type: string) => {
    ...
    if (t.includes('word') || t.includes('document'))
      return <Icons.Description className="w-5 h-5 text-blue-500" />;
    if (t.includes('excel') || t.includes('spreadsheet') || t.includes('csv'))
      return <Icons.Description className="w-5 h-5 text-emerald-600" />;
      ...
      {loading ? (
        ...
        >
          ...
          <p className="text-stone-500 font-medium mb-2">
            Chưa có tài liệu nào học sinh nào được ghi nhận.
          </p>
          <p className="text-[10px] font-black text-stone-400 uppercase tracking-widest italic text-center">
            Tải lên hồ sơ nhập học, bản sao CMND hoặc các chứng chỉ.
          </p>
        </Card>
      ) : (
        <div className="grid grid-cols-1 gap-3">
          {documents.map((doc) => (
            <div
              key={doc.id}
              className="flex items-center gap-5 p-5 bg-white dark:bg-stone-900 border border-stone-100 dark:border-white/5 rounded-[2rem] group hover:shadow-xl hover:border-amber-500/20 transition-all duration-500"
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="font-serif font-black text-stone-900 dark:text-white uppercase tracking-tight truncate">
                  {doc.name}
                </p>
