...
export function SearchModal({ isOpen, onClose }: SearchModalProps) {
  ...
  const [searchResults, setSearchResults] = useState<
    { type: 'feature' | 'data'; name: string; sub?: string; href: string }[]
  >([]);
  ...
  return (
    <>
      <div
        className="fixed inset-0 z-[100] bg-black/80 backdrop-blur-2xl animate-fade-in"
        onClick={onClose}
      />
      <div className="fixed inset-0 md:inset-auto md:top-24 md:left-1/2 md:-translate-x-1/2 md:w-[700px] md:max-h-[80vh] bg-white dark:bg-[#1C1A16] md:rounded-[32px] z-[110] animate-scale-in flex flex-col overflow-hidden shadow-[0_64px_128px_-24px_rgba(0,0,0,0.6)] border border-white/5">
        <div className="flex items-center justify-between p-6 border-b border-white/5 bg-stone-50/50 dark:bg-white/5">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-2xl font-black text-stone-900 dark:text-white tracking-tighter uppercase whitespace-nowrap">
              Tìm kiếm nhanh
            </h2>
          </div>
          <button
        ...
        <div className="p-6 md:p-10">
          ...
          <div
            className="mt-8 overflow-y-auto max-h-[400px] pr-2 custom-scrollbar"
            ...
            {isSearching ? (
              ...
              <div className="grid grid-cols-1 gap-2">
                {searchResults.map((result, idx) => {
                  ...
                  const showHeader = idx === 0 || prev?.type !== result.type;
                ...
                <div className="grid grid-cols-2 gap-3">
                  {[
                    ...
                    >
                      ...
                      <span className="text-sm font-black text-stone-600 dark:text-stone-300 group-hover:text-amber-600 transition-colors uppercase tracking-tight truncate">
                        {item.name}
                      </span>
                    </button>
                  ))}
