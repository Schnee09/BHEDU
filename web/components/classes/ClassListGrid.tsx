...
export default function ClassListGrid({
  classes,
  searchQuery,
  canEnrollStudents,
  onEnrollClick,
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-6">
      {filtered.map((cls) => (
        <div
          key={cls.id}
          className="group relative bg-white dark:bg-stone-900 rounded-2xl border border-stone-200/80 dark:border-stone-800 shadow-xs hover:shadow-md hover:-translate-y-1 transition-all duration-300 overflow-hidden flex flex-col justify-between h-full"
        >
          {/* Clickable Card Body linking to detail */}
          <Link
            href={routes.classes.detail(cls.id)}
            className="p-4 sm:p-6 flex-1 flex flex-col justify-between cursor-pointer"
          >
            <div>
                ...
                {cls.status && (
                  <span
                    className={cn(
                      ...
                      cls.status === 'active' &&
                        'bg-emerald-50 text-emerald-600 dark:bg-emerald-500/10 dark:text-emerald-400 border-emerald-100/50 dark:border-emerald-950/20',
                      cls.status === 'inactive' &&
                        'bg-amber-50 text-amber-600 dark:bg-amber-500/10 dark:text-amber-400 border-amber-100/50 dark:border-amber-950/20',
                      cls.status === 'completed' &&
                        'bg-stone-50 text-stone-500 dark:bg-stone-800/40 dark:text-stone-400 border-stone-150 dark:border-stone-800'
                    )}
                  >
                    {cls.status === 'active'
                      ? 'Hoạt động'
              ...
              {/* Class Title */}
              <h2 className="text-base sm:text-lg font-bold text-stone-900 dark:text-white leading-snug mb-1 group-hover:text-amber-500 transition-colors">
                {cls.name}
              </h2>
              ...
              <div className="grid grid-cols-2 gap-x-3 gap-y-2 my-3 pt-3 border-t border-stone-100 dark:border-stone-800/50 text-[11px] text-stone-500 dark:text-stone-400 font-semibold">
                <div className="flex items-center gap-1.5 min-w-0">
                  <User className="w-3.5 h-3.5 text-stone-400 shrink-0" />
                  <span
                    className="truncate"
                    title={cls.teacher ? getDisplayName(cls.teacher) : 'Chưa giao giáo viên'}
                  >
                ...
                return (
                  <div className="space-y-1 pt-0.5">
                    <div className="flex justify-between items-center text-[10px] font-bold text-stone-400 dark:text-stone-500 uppercase tracking-wider">
                      ...
                      <span>
                        {studentCount}/{maxCapacity} HS
                      </span>
                    </div>
                    <div className="h-1.5 bg-stone-100 dark:bg-stone-800 rounded-full overflow-hidden relative">
                      <div
                        className={cn(
          ...
          {/* Action Footer */}
          <div className="px-4 pb-4 sm:px-6 sm:pb-6 pt-3 sm:pt-4 border-t border-stone-100 dark:border-stone-800/60 flex items-center justify-between gap-2">
