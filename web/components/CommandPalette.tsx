...
export default function CommandPalette() {
    ...
    const searchTimeout = setTimeout(async () => {
      ...
      try {
        ...
        if (data.students) {
          data.students
            .slice(0, 5)
            .forEach((s: { id: string; full_name: string; email?: string }) => {
              apiResults.push({
                id: `student-${s.id}`,
                type: 'student',
                title: s.full_name,
        ...
        if (data.classes) {
          data.classes
            .slice(0, 5)
            .forEach((c: { id: string; name: string; grade_level?: string }) => {
              apiResults.push({
                id: `class-${c.id}`,
                type: 'class',
                title: c.name,
  ...
  return createPortal(
    <div className="fixed inset-0 z-[1100] flex items-start justify-center pt-[20vh]">
      {/* Backdrop */}
      <div
      ...
      {/* Modal */}
      <div className="relative z-[1100] w-full max-w-xl bg-white dark:bg-gray-800 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 overflow-hidden animate-in fade-in zoom-in-95">
        {/* Search Input */}
