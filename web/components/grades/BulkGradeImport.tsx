...
export default function BulkGradeImport({
  ...
  const parseExcelFile = async (file: File) => {
    try {
      ...
      if (!worksheet) {
        alert('Không thấy sheet dữ liệu');
        return;
      }
      ...
      <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-700 bg-gradient-to-r from-emerald-600 to-teal-600">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-white">Import điểm từ Excel</h2>
              <p className="text-white/80 text-sm">Tải lên file Excel để nhập điểm hàng loạt</p>
            </div>
          </div>
          {onClose && (
            <button
        ...
        {step === 'upload' && (
          <div className="space-y-6">
            ...
            <div className="flex items-center justify-between p-4 bg-blue-50 dark:bg-blue-900/20 rounded-xl border border-blue-200 dark:border-blue-800">
              <div>
                <p className="font-medium text-blue-900 dark:text-blue-100">Tải file mẫu</p>
            ...
            <label className="flex flex-col items-center justify-center w-full h-64 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-xl cursor-pointer bg-gray-50 dark:bg-gray-900/50 hover:bg-gray-100 dark:hover:bg-gray-900 transition-colors">
              <div className="flex flex-col items-center justify-center pt-5 pb-6">
                ...
                <p className="text-sm text-gray-500">Chỉ hỗ trợ file .xlsx, .xls</p>
              </div>
              <input
                type="file"
                className="hidden"
          ...
          <div className="space-y-4">
              ...
              <table className="w-full text-sm">
                <thead className="bg-gray-50 dark:bg-gray-900 sticky top-0">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-left font-semibold">Tên HS</th>
                ...
                <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                  {parsedData.map((grade, idx) => (
                    <tr key={idx} className={grade.isValid ? '' : 'bg-red-50 dark:bg-red-900/20'}>
                      <td className="px-4 py-3 font-mono">{grade.studentId}</td>
                      <td className="px-4 py-3">{grade.studentName}</td>
                      <td className="px-4 py-3 font-semibold">{grade.score}</td>
            ...
            <div className="flex items-center justify-between pt-4 border-t">
              ...
              <button
                ...
                className="flex items-center gap-2 px-6 py-2.5 bg-emerald-600 hover:bg-emerald-700 disabled:bg-gray-400 text-white rounded-lg font-semibold transition-colors"
              >
                {importing ? (
                  <>
