...
export default function StudentStatusPanel({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Note: parent page is a server component; we keep local state updated.
      // If you want server-refresh, we can add a refresh link using Next router.
      setReason('')
    } catch (err) {
      showToast.dismiss(toastId)
      console.error(err)
    ...
    <div className="space-y-4">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm text-gray-600 mt-1">Use this to archive, suspend, or mark a student as graduated.</p>
        </div>
      ...
      <div className="rounded-lg border border-gray-200 bg-white p-4 space-y-3">
        <div>
          ...
          <select
            ...
            className="w-full md:w-80 rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 disabled:bg-gray-100"
          >
          ...
          <p className="text-xs text-gray-600 mt-2">{cfg.description}</p>
        </div>
        ...
        {isAdmin && (
          <div className="flex items-center gap-2">
            <button
              ...
              className="px-4 py-2 bg-stone-900 text-white rounded-lg hover:bg-stone-800 text-sm font-medium disabled:bg-stone-300 disabled:cursor-not-allowed"
            >
              {saving ? 'Saving...' : 'Save Status'}
            </button>
            {changed && !saving && (
              <span className="text-xs text-gray-600">Change will take effect immediately.</span>
            )}
          </div>
        )}
      </div>
      ...
      <div className="rounded-lg border border-gray-200 bg-gray-50 p-4">
        <h3 className="font-semibold text-gray-900 mb-2">Workflow notes</h3>
