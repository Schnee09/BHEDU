...
import {
    ...
    NavLink,
    getNavLabel,
} from "@/lib/auth/navigation.config";
    ...
    const renderNavLink = (link: NavLink, inGroup = false) => {
        ...
        return (
            ...
            >
                ...
                <link.icon size={inGroup ? 15 : 18} className={cn(
                    "shrink-0 transition-all duration-200",
                    isActive ? "text-amber-500 scale-110 drop-shadow-[0_0_6px_rgba(245,166,35,0.4)]" : "text-stone-400 group-hover:text-stone-600 dark:group-hover:text-stone-300 group-hover:scale-110"
                )} />

                {!isCollapsed && (
                    <span className={cn(
                        "truncate transition-colors tracking-tight text-sm",
                        isActive ? "text-amber-700 dark:text-amber-400 font-semibold" : "text-stone-500 dark:text-stone-400 group-hover:text-stone-900 dark:group-hover:text-white font-medium"
                    )}>{label}</span>
                )}
            </Link>
        );
            ...
            <div key={group.label} className="space-y-1">
                ...
                <button
                    ...
                    className={cn(
                        ...
                        hasActiveLink ? "bg-amber-500/5 border border-amber-500/10" : "hover:bg-white/50 dark:hover:bg-white/5 border border-transparent hover:border-stone-200 dark:hover:border-white/10",
                        isExpanded ? "mb-1" : ""
                    )}
                ...
                >
                    ...
                    <group.icon size={20} className={cn(
                        ...
                        hasActiveLink ? "text-amber-500 scale-110" : "text-stone-400 group-hover:text-stone-600 dark:group-hover:text-stone-300 group-hover:scale-110"
                    )} />
                    ...
                    {!isCollapsed && (
                        <>
                            <span className={cn(
                                ...
                                hasActiveLink ? "text-amber-700 dark:text-amber-400 font-bold" : "text-stone-600 dark:text-stone-400 group-hover:text-stone-900 dark:group-hover:text-white font-semibold"
                            )}>{group.label}</span>
                            {isExpanded ? (
                                <ChevronDown size={16} className="shrink-0 text-stone-400 group-hover:text-stone-600 dark:group-hover:text-stone-300 transition-transform duration-200 rotate-180" />
                            ) : (
                                <ChevronRight size={16} className="shrink-0 text-stone-400 group-hover:text-stone-600 dark:group-hover:text-stone-300 transition-transform duration-200" />
                            )}
                        </>
                    )}
                </button>
