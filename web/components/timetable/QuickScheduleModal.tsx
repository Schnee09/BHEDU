...
export default function QuickScheduleModal({
  ...
  useEffect(() => {
    ...
    if (initialData) {
      setDayOfWeek(initialData.dayIndex ?? 0);
    ...
    try {
      ...
      if (!res.ok) {
        if (res.status === 409 || data.code === "CONFLICT") {
          setConflictWarning(data.error || "Phòng học hoặc Giáo viên bị trùng lịch vào khung giờ này.");
      ...
      toast.success("Xếp lịch thành công", "Đã thêm tiết học mới vào Thời Khóa Biểu!");
      onSuccess();
      onClose();
    } catch (err: any) {
      ...
      <form onSubmit={handleSave} className="space-y-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <select
              ...
              className="w-full px-4 py-3 bg-stone-50 dark:bg-stone-800/80 rounded-2xl border border-stone-200 dark:border-white/10 text-sm font-bold text-stone-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 transition-all cursor-pointer"
            >
        ...
        <div className="flex items-center justify-end gap-3 pt-3 border-t border-stone-200/80 dark:border-white/10">
          <button
            ...
            onClick={onClose}
            disabled={saving}
            className="px-5 py-2.5 rounded-xl border border-stone-200 dark:border-white/10 text-xs font-black uppercase text-stone-600 dark:text-stone-300 hover:bg-stone-100 dark:hover:bg-stone-800 transition-all"
          >
            Hủy
          </button>
          <button
            type="submit"
            disabled={saving}
            className="px-6 py-2.5 rounded-xl bg-amber-500 hover:bg-amber-600 text-white text-xs font-black uppercase tracking-wider shadow-md hover:shadow-amber-500/20 active:scale-95 transition-all flex items-center gap-2 disabled:opacity-50"
          >
            {saving ? (
              <>
