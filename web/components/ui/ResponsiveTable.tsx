...
export function ResponsiveTable({ children, mobileView, className = '' }: ResponsiveTableProps) {
  if (!mobileView) {
    ...
    return (
      <div className={`overflow-x-auto -mx-4 px-4 sm:mx-0 sm:px-0 ${className}`}>{children}</div>
    );
  }
  ...
  return (
    <>
      {/* Mobile view */}
      <div className={`md:hidden ${className}`}>{mobileView}</div>
      {/* Desktop view */}
  ...
  return (
    <div
      ...
      className={cn(
        ...
        rounded-[24px] md:rounded-[32px] p-6
        shadow-sm active:scale-[0.97] transition-all duration-300 ease-[cubic-bezier(0.16,1,0.3,1)]
    ...
    >
      ...
      <div className="flex items-start justify-between gap-3 mb-4">
        <div className="flex-1 min-w-0">
          <div className="font-extrabold text-stone-900 dark:text-stone-100 text-lg leading-tight truncate">
            {title}
          </div>
          {subtitle && (
            <div className="text-sm font-semibold text-stone-500 dark:text-stone-400 mt-1 flex items-center gap-1.5 transition-colors">
              {subtitle}
            </div>
          )}
        </div>
        {status && (
          <span
            className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider flex-shrink-0 ${statusColors[status.color]}`}
          >
            {status.label}
          </span>
        )}
      ...
      {/* Fields */}
      <div className="space-y-3 bg-stone-50/50 dark:bg-white/[0.02] rounded-2xl p-4">
        {fields.map((field, index) => (
          <div key={index} className="flex justify-between items-center text-sm">
            <span className="text-stone-500 dark:text-stone-400 text-xs font-bold uppercase tracking-wider">
              {field.label}
            </span>
      ...
      {/* Actions */}
      {actions && (
        <div className="flex gap-2 mt-5 pt-4 border-t border-stone-200 dark:border-stone-800">
          {actions}
        </div>
      )}
    </div>
