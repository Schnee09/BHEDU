...
async function verifySearchAndIdentification() {
  ...
  try {
    console.log(`📝 Step 1: Creating student with UID=${studentData.student_code} and CID=${studentData.student_id}...`);
    ...
    if (searchUid.data.length > 0 && searchUid.data[0].id === studentId) {
    ...
    if (searchCid.data.length > 0 && searchCid.data[0].id === studentId) {
