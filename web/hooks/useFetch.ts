...
import { ApiErrorResponse, ApiPaginatedResponse, ApiResponse } from '@/lib/api/types';
...
export function useFetch<T = any>(
  ...
  const fetchData = useCallback(async () => {
    ...
    // Check cache first
    const cleanUrl = (url?.split('?')[0] || url || '') as string;
    ...
    try {
      ...
      const hasAnyMetadataKey = (payload: AnyRecord) =>
        metadataKeys.some((k) => Object.prototype.hasOwnProperty.call(payload, k));
      ...
      if (Array.isArray(result)) {
        ...
        if (hasData) {
          ...
          if (hasMeta) {
            ...
            if ((result as any).pagination?.total !== undefined && !(result as any).total) {
              fetchedData = {
                ...result,
            ...
            keys.length > (Object.prototype.hasOwnProperty.call(result, 'success') ? 2 : 1)
          ) {
